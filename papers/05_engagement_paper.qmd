...
frame_eng_wide <- frame_eng |>
  select(frame, present, mean_eng) |>
  pivot_wider(names_from = present, values_from = mean_eng, names_prefix = "present_") |>
  mutate(
...
kable(frame_eng_wide |>
        select(frame, present_FALSE, present_TRUE, diff, ratio) |>
        mutate(across(c(present_FALSE, present_TRUE, diff), ~round(.x, 1))),
...
m1_frames <- m1_tidy |>
  ...
  select(term, estimate, conf.low, conf.high, p_display, sig)
...
m1_media <- m1_tidy |>
  ...
  arrange(desc(estimate)) |>
  select(term, estimate, conf.low, conf.high, p_display)
...
kable(m2_compare |> select(-p.value),
...
extract_frame_irr <- function(model, label) {
  ...
  tidy(model, conf.int = TRUE, exponentiate = TRUE) |>
    ...
    select(source, term, estimate, conf.low, conf.high, p_display)
}
